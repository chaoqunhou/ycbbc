...
class Shop extends BaseService implements IShop
{
    ...
    /**
     * @param $shop_id 商铺ID
     * @return array|false|\PDOStatement|string|think\Collection
    ...
    {
    $shop_ad = new NsShopAdModel();
    $list = $shop_ad->where(['shop_id' => $shop_id]) -> select();
    if (empty($list)){
        return [];
    }
    ...
    public function addShopAd($ad_image, $link_url, $sort, $type, $background)
    {
        ...
        $shopAdList = $this ->getShopPicList($shop_id);
        if (count($shopAdList)>=5){
            return -1;
        }
    ...
    */
    ...
    public function createInvitecode(){
        ...
        $uiid = $user_info['instance_id'];
        $ic_data = db('sys_invite_code')->where('shop_id',$uiid)->find();
//        $ic_data = db('sys_invite_code')->where('shop_id',$uiid)->select();
//        foreach ($ic_data as $k => $v){
//            $arr[] = $v['invite_code'];
//        }
        $shop_info = $this->getShopDetail($uiid);
        $invite_code = '';
        if(!empty($shop_info['base_info']) && empty($ic_data))//店铺有唯一邀请码
        {
            ...
            $help = 0;
            if ($uiid < 10) {
                $invite_code = $help.$help.$help.$uiid.$str;
            }elseif($uiid < 100 && $uiid >= 10){
                $invite_code = $help.$help.$uiid.$str;
            }elseif($uiid < 1000 && $uiid >= 100){
                $invite_code = $help.$uiid.$str;
            }elseif($uiid < 10000 && $uiid >= 1000){
                $invite_code = $uiid.$str;
            }
            $data = ['shop_id' => $uiid, 'user_id' => $user_info['uid'],'invite_code' => $invite_code,];
            db('sys_invite_code')->insert($data);
//            $icd = db('sys_invite_code')->where('invite_code',$invite_code)->find();
//            if (empty($icd)){
//                db('sys_invite_code')->insert($data);
//            }
            $usb = db('sys_shop_user_bund')->where('shop_id',$uiid)->where('user_id',$user_info['uid'])->find();
            $usb_data = ['shop_id' => $uiid, 'user_id' => $user_info['uid'],'invite_code' => $invite_code,'record'=>2];
            if (empty($usb)){
                db('sys_shop_user_bund')->insert($usb_data);
            }
//            $record = (empty($icd) && empty($usb)) ? 1 :0;
//            if (!in_array($invite_code,$arr)){
//                db('sys_invite_code')->insert($data);
//            }
        }
        $icd = db('sys_invite_code')->where('shop_id',$uiid)->find();
        if(!empty($icd)){
            $invite_code = $icd['invite_code'];
            try {
                $invite_qrcode = getQRcode($invite_code, 'upload/invite_qrcode', 'invite_qrcode_' . $invite_code);
            } catch (\Exception $e) {
                return AjaxReturn(UPLOAD_FILE_ERROR);
            }
            $this->invite_QRcode_make($invite_code, $invite_qrcode);
        }
        return $icd;
    }
     ...
     * (non-PHPdoc)
     */
    function invite_QRcode_make($invitecode, $ic_qrcode)
    {
        $data = array(
            'qrcode_src' => $ic_qrcode
        );
        $result = db('sys_invite_code')->where('invite_code', $invitecode)->setField(
            'qrcode_src',$ic_qrcode
        );
        if ($result > 0) {
            return SUCCESS;
