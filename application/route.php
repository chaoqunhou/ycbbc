...
//检测后台系统模块
   ...
   /************************************************************************************************检测入口文件*****************************************************************************************/
    ...
    $is_api 	=  strpos(Request::instance()->server('SCRIPT_NAME'), 'api.php');
    $is_Action 	=  strpos(Request::instance()->server('REQUEST_URI'), 'Action');
    if($alipay)
    {
...
//        ],
	...
	if($version==null) $version = "v1";
		$api_route = [
			'__rest__'=> [':version/:controller' => "api/{$version}.:controller"],
			':version/:controller/:action/:id' => ["api/{$version}.:controller/:action",['method' => 'post']],
		];
    /*****************************************************************************************************普通路由设置结束******************************************************************************/
    if($is_api)
    {
		if ($is_Action){
			return [
				':version/:controller/:action' => ["api/{$version}.:controller/:action",['method' => 'post']],
			];
		}else{
			return $api_route;
		}
